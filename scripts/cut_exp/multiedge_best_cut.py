...
def gen_data_file(
    ...
    num_sink = sum([len(sg.g.get_sinks_and_cloud_vertices()) for sg in graph_list])
    op_index_map = dict()
    index_op_map = dict()
    index_counter = 0
    for sg in graph_list:
        for op in sg.g.get_sources():
            ...
            index_counter += 1
    for sg in graph_list:
        for op in sg.g.get_operators_colored_not_cloud():
            op_index_map[op.uuid] = index_counter
            index_op_map[index_counter] = (op.uuid, sg.idx)
            index_counter += 1
    for sg in graph_list:
        for op in sg.g.get_sinks_and_cloud_vertices():
            op_index_map[op.uuid] = index_counter
            index_op_map[index_counter] = (op.uuid, sg.idx)
            index_counter += 1
