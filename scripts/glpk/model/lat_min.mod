...
var net_node_cores_occupied_num_real{net_nodes}, integer, >=0;
var u1{net_nodes}, binary;
var u2{net_nodes}, binary;
...
## 延迟相关
subject to
    net_node_cores_occupied_num_real_{i in net_nodes}:
        net_node_cores_occupied_num_real[i] = sum{j in flow_nodes} f[j,i];
    max_1{i in net_nodes}:
        net_node_cores_occupied_num[i] >= net_node_cores_occupied_num_real[i];
    max_2{i in net_nodes}:
        net_node_cores_occupied_num[i] >= cores[i];
    max_4{i in net_nodes}:
        net_node_cores_occupied_num[i] <= net_node_cores_occupied_num_real[i] + 2100000000 * (1 - u1[i]);
    max_5{i in net_nodes}:
        net_node_cores_occupied_num[i] <= cores[i] + 2100000000 * (1 - u2[i]);
    max_6{i in net_nodes}:
        u1[i] + u2[i] >= 1;
subject to
    compute_power_ratio_{i in net_nodes}:
        compute_power_ratio[i] = net_node_cores_occupied_num_real[i] / (cores[i] * mips[i]);
